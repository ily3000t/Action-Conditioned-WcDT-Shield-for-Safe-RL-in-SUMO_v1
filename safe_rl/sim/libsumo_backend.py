...
class LibsumoBackend(ISumoBackend):
    ...
    def start(self):
        ...
        try:
            ...
            self._use_mock = True
            self._started = True
            self._record_session_event("start_mock_backend", exc=exc, reason="libsumo_import_failed")
            return
        ...
        try:
            ...
            raise self._make_start_error("libsumo initial start failed", exc, self.config.random_seed) from exc

        self._use_mock = False
        self._started = True
        _LOGGER.info(
            "libsumo backend started in real SUMO mode with cfg=%s, log=%s",
            cfg_path,
    ...
    def reset(self, seed: Optional[int] = None) -> SceneState:
        ...
        try:
            ...
            raise self._make_reset_error("libsumo reset failed", exc, seed_value) from exc
        ...
        if warmup_status["fatal"]:
            ...
            if restart_attempted:
                ...
                raise self._make_reset_error("libsumo reset failed after warmup connection loss", exc, seed_value)
            ...
            try:
                ...
                raise self._make_reset_error("libsumo reset warmup restart failed", exc, seed_value) from exc
            ...
            if warmup_status["fatal"]:
                ...
                raise self._make_reset_error("libsumo reset failed after warmup retry", exc, seed_value)
    ...
    def step(self, action_id: int) -> BackendStepResult:
        ...
        try:
            ...
            if self._is_fatal_libsumo_error(exc):
    ...
    def close(self):
        if not self._started:
            return
        if not self._use_mock and self._libsumo is not None and self._session_active:
            try:
                self._libsumo.close()
            except Exception:
                pass
    ...
    def _resolve_runtime_log_path(self) -> Path:
        ...
        log_dir.mkdir(parents=True, exist_ok=True)
        return log_dir / "libsumo_runtime.log"
    ...
    def _warmup_after_reset(self) -> Dict[str, bool]:
        ...
        try:
            ...
            if self._is_fatal_libsumo_error(exc):
                ...
                _LOGGER.warning(
                    "libsumo reset warmup lost SUMO connection (%s). See log: %s",
                    exc,
                    self.runtime_log_path,
                )
    ...
    def _handle_fatal_step(self, exc: Exception, action_meta: dict) -> BackendStepResult:
        ...
        self._last_risk_meta = None
        _LOGGER.warning(
            "SUMO closed libsumo connection during simulationStep (%s). Episode terminated early. Log: %s",
            exc,
            self.runtime_log_path,
        )
    ...
    def _safe_sim_time(self) -> float:
        ...
        try:
            if self._libsumo is not None and self._session_active:
                return float(self._libsumo.simulation.getTime())
        except Exception:
            pass
        if self._last_scene is not None:
    ...
    def _is_fatal_libsumo_error(self, exc: Exception) -> bool:
        fatal_candidates = [
            getattr(self._libsumo, "FatalTraCIError", None),
            getattr(getattr(self._libsumo, "exceptions", None), "FatalTraCIError", None),
        ]
        for fatal_cls in fatal_candidates:
