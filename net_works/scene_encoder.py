...
class SceneEncoder(nn.Module):
    def __init__(
        ...
        # Keep dimensions consistent with actual his_step so both his_step=10 and his_step=11 work.
        linear_input_dim = (his_step - 1) * 5 + embedding_dim + embedding_dim
        other_conditional_dim = (his_step - 1) * 5 + embedding_dim + embedding_dim
        traffic_light_conditional_dim = his_step + embedding_dim
        ...
        self.other_agent_former = OtherFeatureFormer(
            block_num=other_agent_depth,
            input_dim=dim,
            conditional_dim=other_conditional_dim,
        )
        self.map_former = OtherFeatureFormer(block_num=map_feature_depth, input_dim=dim, conditional_dim=embedding_dim)
        self.traffic_light_former = OtherFeatureFormer(
            block_num=traffic_light_depth,
            input_dim=dim,
            conditional_dim=traffic_light_conditional_dim,
        )
    ...
    def forward(
        ...
        # map point transformer
        x = self.map_former(x, lane_list)
        # traffic light transformer
